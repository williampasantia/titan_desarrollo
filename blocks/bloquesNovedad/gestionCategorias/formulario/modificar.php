...
class Formulario {
	...
	function formulario() {
		...
		$atributos ['tipoEtiqueta'] = 'inicio';
		$atributos ["leyenda"] = "REGISTRO NUEVA CATEGORÍA";
		...
		unset ( $atributos );
		{
			...
			$atributos1 ['cadena_sql'] = $this->miSql->getCadenaSql ( "buscarVerdetallexCategoria", $datos );
		...
		$datos2 = array (
				'id' => $matrizItems2 [0] [0],
				'nombre' => $matrizItems2 [0] [1],
				'descripcion' => $matrizItems2 [0] [2],
				'estado' => $matrizItems2 [0] [3],
				'ley' => $id_ldn [0] [0] 
		);
		
		$atributos2 ['cadena_sql'] = $this->miSql->getCadenaSql ( "versiones", $datos2 );
		$primerRecursoDB->ejecutarAcceso ( $atributos2 ['cadena_sql'], "busqueda" );
		// -----------------FIN CONTROL: Botón -----------------------------------------------------------
