...
class Formulario {
	...
	function formulario() {
		...
		$atributos ['tipoEtiqueta'] = 'inicio';
		$atributos ["leyenda"] = "GESTIÓN DE CATEGORÍAS";
		...
		// -----------------CONTROL: Botón ----------------------------------------------------------------
		$esteCampo = 'botonRegistrarCategoria';
		$atributos ["id"] = $esteCampo;
		...
		$atributos ['cadena_sql'] = $this->miSql->getCadenaSql ( "buscarRegistroxActivo" );
		$matrizItems = $primerRecursoDB->ejecutarAcceso ( $atributos ['cadena_sql'], "busqueda" );
		$longitud = count ( $matrizItems );
		
		$i = 0;
		
		echo '<table id="tablaReporte" class="display" cellspacing="0" width="100%"> ' . '<thead style="display: table-row-group"><tr><th>' . "NOMBRE" . '</th><th>' . "DESCRIPCIÓN" . '</th> <th>' . "LEY" . '</th><th>' . "VER CONCEPTOS" . '</th><th>' . "MODIFICAR" . '</th><th>' . "INACTIVAR" . '</th></tr></thead>
